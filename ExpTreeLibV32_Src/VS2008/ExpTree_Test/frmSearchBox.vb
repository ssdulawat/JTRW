...
''' <summary>
''' This Form is a modified version of frmThread. It contains a SearchBox for finding files in
''' the currently selected Folder and its' sub-Folders whose name matches the search pattern
''' given in the SearchBox. Searching is done only when the button next to the SearchBox is Clicked.
''' 
''' NOTE WELL: The entire Dynamic update feature has been disabled(commented out) for this Form. It does not 
''' listen for Change Notifications and therefore does not dynamically update its' display when
''' something changes!!! I suspect that could be added back in with little trouble, but, this
''' Form did not warrant the additional development and testing time, so I left that out!!!
''' </summary>
''' <remarks>
...
#Region "   Form Close Methods"
    ...
    Private Sub frmSearchBox_FormClosing(ByVal sender As Object, ByVal e As System.Windows.Forms.FormClosingEventArgs) Handles Me.FormClosing  '7/1/2012
...
#Region "   Form Load/VisibleChanged, lv1 HandleCreated"
    ...
    Private Sub frmSearchBox_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
...
#Region "   ExplorerTree Event Handling -- AfterNodeSelect"
    Private Sub AfterNodeSelect(ByVal pathName As String, ByVal CSI As CShItem) _
        ...
        If LastSelectedCSI IsNot Nothing AndAlso LastSelectedCSI IsNot CSI Then     'Clear excess File CShItems
        ...
        ShowItemsInListView(combList, CSI)
        LastSelectedCSI = CSI
        lv1.Tag = LastSelectedCSI           '7/5/2012   For ClvDropWapper
        Cursor = Cursors.Default
    End Sub

    Private Function ShowItemsInListView(ByVal comblist As ArrayList, ByVal CSI As CShItem, Optional ByVal InSearch As Boolean = False) As Integer
        Dim TotalItems As Integer = comblist.Count
        If TotalItems > 0 Then
            ...
            If InSearch Then
                InitialFillLim = comblist.Count - 1
            End If
...
#Region "   Filter Search Box Routines"

    Private LastDispWasSearch As Boolean = False        'Used to restore to normal state

    Private Sub tsbSearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tsbSearch.Click
        Dim Lst As New ArrayList
        GetFilteredFiles(LastSelectedCSI, Lst)
        LastDispWasSearch = True
        ShowItemsInListView(Lst, LastSelectedCSI, True)
    End Sub

    Private Sub GetFilteredFiles(ByVal Folder As CShItem, ByRef FLst As ArrayList)
        FLst.AddRange(Folder.GetFiles(tstbFilter.Text))
        'For Each F As CShItem In Folder.GetFiles
        '    If F.DisplayName.ToLower Like tstbFilter.Text.ToLower Then FLst.Add(F)
        'Next
        For Each D As CShItem In Folder.GetDirectories
            GetFilteredFiles(D, FLst)
        Next
    End Sub
    Private Sub tstbFilter_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles tstbFilter.TextChanged
        If tstbFilter.Text.Trim.Length > 0 Then
            tsbSearch.Enabled = True
        ElseIf LastDispWasSearch AndAlso LastSelectedCSI IsNot Nothing Then
            Dim combList As ArrayList = LastSelectedCSI.GetItems
            ShowItemsInListView(combList, LastSelectedCSI, False)
            LastDispWasSearch = False
        End If
    End Sub
#End Region
